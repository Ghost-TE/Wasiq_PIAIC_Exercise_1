//Question 37
// Large Shirts: Modify the make_shirt() function so that shirts are large by default with a 
// message that reads I love JavaScript. Make a large shirt and a medium shirt with the 
// default message, and a shirt of any size with a different message.
function make_shirt(size, message) {
    console.log("The size of shirt " + size + " & the typo written on it is: " + message);
}
make_shirt("Large", "I Love JavaScript");
make_shirt("small", "I am Smart");
